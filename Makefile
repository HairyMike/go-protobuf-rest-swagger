...
PROTOC_GEN_GRPC_GATEWAY_VERSION=v2.14.0
...
# URLs for downloading protoc based on architecture and OS
ifeq ($(ARCH),arm64)
  PROTOC_INSTALL_CMD := brew install protobuf
else ifeq ($(ARCH),x86_64)
  # Explicit URL for the Linux x86_64 version of protoc
  ...
  PROTOC_INSTALL_CMD := curl -LO $(PROTOC_ZIP_URL) && unzip -o protoc-$(PROTOC_VERSION)-linux-x86_64.zip -d $(HOME)/local && rm protoc-$(PROTOC_VERSION)-linux-x86_64.zip
else
...
# Update the PATH and INCLUDE_DIR for the current session
export PATH := $(BIN_DIR):$(PATH)
export PROTOC_INCLUDE := $(INCLUDE_DIR)
...
install_protoc_gen_openapi:
	@if ! [ -x "$(BIN_DIR)/protoc-gen-openapiv2" ]; then \
		...
		GOBIN=$(BIN_DIR) go install github.com/grpc-ecosystem/grpc-gateway/v2/protoc-gen-openapiv2@$(PROTOC_GEN_OPENAPI_VERSION); \
	else \
